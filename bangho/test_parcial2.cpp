...
int main(){
  #define MAYOR(may,i,j,mat) (i==0&&j==0?may=mat[i][j]:(mat[i][j]>may?may=mat[i][j]:mat[i][j]==mat[i][j]))
  ...
  for (i=0; i < n; i++){
    for(j = 0; j <m ; j++){
      if(mat[i][j] == may){
        cout << "se encontro el mayor " << may << " en la posicion " <<  "fila " << (i+1)  <<" columna " << (j+1) << endl;
      }
